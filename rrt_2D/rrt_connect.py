...
def main(start=(2,2), goal=(49,24), terrain=1, analysis=False, iterations=1, n=5000):
    ...
    if analysis:
        ...
        for i in range(iterations):
            rrt_conn = RrtConnect(x_start, x_goal, 0.8, 0.05, n)
            time.start()
            path = rrt_conn.planning()
            time.stop()
            elapsed_time.append(time.elapsed_time)
            path_len.append(rrt_conn.utils.path_length(path))
            path_count.append(len(path))
            # print(time.elapsed_time)
        fig = rrt_conn.plotting.animation_connect(rrt_conn.V1, rrt_conn.V2, path, "RRT_CONNECT")
        print("elapsed_time: ")
        ...
        time.stop()
        print(time)
        print("Path Length: ", rrt_conn.utils.path_length(path))
