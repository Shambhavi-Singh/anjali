...
import env, plotting, utils, queues, timer
...
class RrtStar:
    def __init__(self, x_start, x_goal, step_len,
                 goal_sample_rate, search_radius, iter_max, analysis=False):
    ...
    def planning(self):
        ...
        self.timer.stop()
        if self.analysis:
            # if (math.dist(self.path[-1], self.path[-2]) <= self.step_len):
...
def main(start=(2,2), goal=(49,24), terrain=1, analysis=False, iterations=1, n=1000):
    ...
    if analysis:
        ...
        for i in range(iterations):
            rrt_star = RrtStar(x_start, x_goal, 1, 0.10, 20, n, True)
            rrt_star.planning()
        fig = rrt_star.plotting.animation(rrt_star.vertex, rrt_star.path, "rrt*, N = " + str(rrt_star.iter_max))
        print("elapsed time:")
        ...
        # Demo part
        rrt_star = RrtStar(x_start, x_goal, 1, 0.10, 20, n, False)
        rrt_star.planning()
        fig = rrt_star.plotting.animation(rrt_star.vertex, rrt_star.path, "rrt*, N = " + str(rrt_star.iter_max))
