...
class DynamicRrt:
    def __init__(self, s_start, s_goal, step_len, goal_sample_rate, waypoint_sample_rate, iter_max, analysis=False, animate=True):
        ...
        for i in range(self.iter_max):
            ...
            if node_new and not self.utils.is_collision(node_near, node_new):
                ...
                if dist <= self.step_len:
                    ...
                    self.timer.stop()
                    print(self.timer)
                    print("Path Length: ", self.utils.path_length(path))
                    ...
                    if not self.analysis:
                        ...
                        path_len.append(self.utils.path_length(path))
                        elapsed_time.append(self.timer.elapsed_time)
                        path_count.append(len(path))
                        self.path = path
                        self.waypoint = self.extract_waypoint(node_new)
                        self.on_press(None)
                    return
    ...
    def on_press(self, event):
        if self.analysis:
            x, y = self.path[len(self.path)//2] if self.path else (25, 15)
        else:
            x, y = event.xdata, event.ydata
        if not x or not y or x < 0 or x > 50 or y < 0 or y > 30:
            ...
            if self.is_path_invalid():
                ...
                if self.animate:
                    ...
                    self.plot_path(self.path, color='red')
                    self.plot_vertex_new()
                    self.plot_path(path)
                    plt.show()
                if self.analysis:
                    new_path_len.append(self.utils.path_length(path))
                    new_elapsed_time.append(self.timer.elapsed_time)
                    new_path_count.append(len(path))
                self.vertex_new = []
                self.path = path
    ...
    def plot_visited(self, animation=True):
        if animation:
            ...
            for node in self.vertex:
                count += 1
                if node.parent:
                    plt.plot([node.parent.x, node.x], [node.parent.y, node.y], plotting.Plotting.visited_path_color)
    ...
    @staticmethod
    def plot_path(path, color=plotting.Plotting.path_color):
...
def main(start=(2,2), goal=(49,24), terrain=1, analysis=False, iterations=1, n=5000):
    ...
    if analysis:
        ...
        path_len.clear()
        new_path_len.clear()
        new_elapsed_time.clear()
        path_count.clear()
        new_path_count.clear()
        for i in range(iterations-1):
            drrt = DynamicRrt(x_start, x_goal, 0.5, 0.1, 0.6, n, analysis=True, animate=False)
            drrt.planning()
        drrt = DynamicRrt(x_start, x_goal, 0.5, 0.1, 0.6, n, analysis=True, animate=True)
        drrt.planning()
        print("elapsed time:")
        ...
        for i in path_len:
            print(i)
        print("new elapsed time:")
        for i in new_elapsed_time:
            print(i)
        print("new path len:")
        for i in new_path_len:
            print(i)
        print ("path nodes: ")
        for i in path_count:
            print(i)
        print ("new path nodes: ")
        for i in new_path_count:
            print(i)
    else:
        # Demo part
        drrt = DynamicRrt(x_start, x_goal, 0.5, 0.1, 0.6, n, analysis=False, animate=True)
        drrt.planning()
    return drrt.fig
